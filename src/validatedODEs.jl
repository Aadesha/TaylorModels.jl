...
function validated_step!(f!, t::Taylor1{T},
        ...
        q0::IntervalBox{N,T}, q0box::IntervalBox{N,T},
        nsteps::Int, abstol::T, parse_eqs::Bool,
    ...
    TaylorIntegration.__jetcoeffs!(Val(parse_eqs), f!, tI, xI, dxI, xauxI)
    ...
    for nchecks = 1:25
        ...
        Δ = remainder_taylorstep!(f!, t, x, dx, xI, dxI, δq_norm, δtI)
        rem .= rem_old .+ Δ
...
function validated_integ(f!, qq0::AbstractArray{T,1}, δq0::IntervalBox{N,T},
    ...
    while t0 < tmax
        ...
        δt = validated_step!(f!, t, x, dx, xaux, tI, xI, dxI, xauxI,
            ...
            nsteps, abstol, parse_eqs, check_property)
